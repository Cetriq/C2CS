...
1. **What am I looking at?** The agent requests the semantic model and receives a
   document. Before anything else it reads the header: `kind: contract` (normative — what
   *shall* hold, not a hypothesis), `c2cs: "0.2"`, `registry: "0.1"`. The document type
   comes first, because it determines the epistemic weight of everything that follows.
   *Fields: kind, c2cs, registry.*

2. **What system is this?** Then `subject` (which service, which version) and
...
3. **What is this system allowed to do?** The agent reads `capabilities` (two outbound
...
4. **Evaluate the requested change.** A call to `api.riskvendor.example` is a new
...
5. **How trustworthy is this picture?** The agent fetches the latest verdict:
...
6. **Advise.** The agent proposes: (a) the feature needs a contract change — a new
